...
def buscar_musculo_no_catalogo(nome_exercicio):
    ...
    try:
        ...
        # Normalizar o nome de busca (remover acentos e converter para minúsculas)
        nome_busca = remover_acentos(nome_exercicio.lower().strip())
        print(f"🔤 Termo de busca normalizado: '{nome_busca}'")
        
        # Mapeamento de músculos em inglês para português (baseado no seu catálogo)
        mapa_musculos = {
            'abdominais': 'Abdômen',
            'abductors': 'Abdutores',
            'adductors': 'Adutores',
        ...
        for ex in catalogo:
            ...
            if nome_busca in nome_catalogo:
                ...
                if primary_muscles and len(primary_muscles) > 0:
                    musculo_original = primary_muscles[0].lower()
                    musculo = mapa_musculos.get(musculo_original, musculo_original.title())
                    print(f"✅ Correspondência parcial (nome contém): {musculo}")
                    return musculo
        ...
        for ex in catalogo:
            ...
            if nome_catalogo in nome_busca:
                ...
                if primary_muscles and len(primary_muscles) > 0:
                    musculo_original = primary_muscles[0].lower()
                    musculo = mapa_musculos.get(musculo_original, musculo_original.title())
                    print(f"✅ Correspondência parcial (catálogo contém): {musculo}")
                    return musculo
